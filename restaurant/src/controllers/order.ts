...
export const getRiderOrderById = TryCatch(async (req, res) => {
  if (req.headers["x-internal-key"] !== process.env.INTERNAL_SERVICE_KEY) {
    return res.status(403).json({
      message: "Forbidden",
    });
...
export const getRiderHistory = TryCatch(async (req, res) => {
  if (req.headers["x-internal-key"] !== process.env.INTERNAL_SERVICE_KEY) {
    return res.status(403).json({
      message: "Forbidden",
    });
  ...
  const todayOrders = history.filter((order) => {
    const finishedAt = order.updatedAt ?? order.createdAt;
    return new Date(finishedAt) >= todayStart;
  });

  const totalEarnings = history.reduce(
    (sum, order) => sum + (order.riderAmount || 0),
    0
  );
